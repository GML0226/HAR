...
def main() -> None:
    ...
    dataset = MultimodalDataset(manifest, data["image_size"], data["image_frames"], data["sequence_length"], labelled=False)
    ...
    model = SixModalHAR(**config["model"], num_classes=data["num_classes"]).to(device, memory_format=torch.channels_last)
    ...
    print(f"Wrote {len(submission)} validated predictions to {output}")
